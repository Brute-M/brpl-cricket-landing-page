  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50 flex flex-col">
        ...
        <div className="h-9 bg-gradient-to-r from-[#FACC15] to-[#f59e0b] text-black overflow-hidden flex items-center shrink-0">
          <style>
            {`
            @keyframes marquee {
        ...
        {/* Main Navbar */}
        <motion.nav
          className={`w-full transition-all duration-300 ${isScrolled ? 'glass py-2' : 'bg-transparent py-4'
            }`}
          initial={{ y: -100 }}
          animate={{ y: 0 }}
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <motion.div
            className="fixed inset-0 z-30 glass pt-40 md:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
